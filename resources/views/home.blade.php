...
@section('content')  
  ...
  <div class="container-fluid mt-3 border-top border-bottom">
    ...
    <a class="text-decoration-none" href="{{ URL::to('/phone/' . $row->parentPhone->phone_id . '/detail/' . $row->phone_details_id) . '/specs/0' }}">
        <h6 class="card-title fw-bold truncate-text">{{ ($row->parentPhone->phone_name . ' ' . $row->parentSpecific->specific_name . ' ' . $row->parentColor->color_name) ?? 'null' }}</h6>
    </a>
